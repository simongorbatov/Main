...
2. Ask Simon, one at a time and only what you can't infer: **What's the very next thing to do?**,
   **When is it due?** and **Priority 1, 2 or 3?**
...
4. Otherwise copy `chiefs/_TEMPLATE.md`, fill the front matter (`chief`, `name`, `status: active`,
   `priority`, `next_action`, `due`, `sources`, `updated: <today>`) and `## Now`. Clear the template's example
   lines.

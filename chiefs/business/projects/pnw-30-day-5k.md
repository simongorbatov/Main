---
chief: business
name: PNW Customs — $5k in 30 days
status: active
priority: 1
next_action: "Ship the 5 unshipped paid orders and get tracking into Shopify"
due: 2026-09-26
sources: [shopify, gmail, calendar]
updated: 2026-09-23
---

## Now

Ship #1073, #1093, #1094, #1095, #1096. Tracking into Shopify. **Overdue — was set for Sat Sept 19.**

## The one number

**$5,000 in sales, Sept 22 → Oct 21.**

## The one rule

Only sell what ships in 3 days.

## Where this started

Store did $5k in May and June. Sept is on pace for ~$2k with zero effort. 5 paid orders unshipped.
70 Notion tasks, none finished. Notion is parked until Nov 1 — this file is the source.

## Daily, 40 min

1. Ship paid orders, answer every message (20 min)
2. One TikTok post (15 min)
3. Write tomorrow's one task here (5 min)

Calendar blocks: weekdays after school and before training; weekends 10:00. Tomorrow's-task block at
21:00. Never schedule over `[School]` or `[Health]`.

## Open

### Overdue — Week 1 slipped
- [ ] Ship #1073, #1093, #1094, #1095, #1096; tracking into Shopify (due 2026-09-19, 120 min)
      — done = 0 paid orders unshipped
- [ ] Reply to Chow (order 1898) and Karter: ship date or refund (due 2026-09-19, 30 min)
      — done = both answered
- [ ] Publish free-shipping theme ($75). Announcement bar: "In-stock parts ship in 2 business days."
      (due 2026-09-20, 45 min) — done = live on pnwcustoms.us
- [ ] Shopify → Checkout → abandoned checkout emails ON (1h + 24h) (due 2026-09-20, 15 min)
- [ ] Unpublish anything that can't ship in 3 days — Collective imports with errors, the 3 refund
      machines (due 2026-09-20, 60 min)

### Week 2
- [ ] Email the 1,056 list: "parts that ship in 2 days," code PNW15, expires Oct 3
      (due 2026-09-26, 60 min) — needs Simon's approval before sending
- [ ] One TikTok account (@pnw__customs). Point the other bios at it. (due 2026-09-26, 30 min)
- [ ] Text 20 buyers who received parts: post your build, tag us (due 2026-09-27, 45 min)
- [ ] Scorecard: 0 orders unshipped over 3 days? (due 2026-09-27, 5 min)

### Week 3
- [ ] Last-call email: code dies tonight (due 2026-10-02, 20 min) — needs Simon's approval
- [ ] Build two bundles from stock: LBX Brake Kit, Cockpit Kit (due 2026-10-03, 90 min)
- [ ] Fix wheelset page: photos, fitment, "ships in 2 days" (due 2026-10-04, 60 min)
- [ ] Scorecard: 5+ PNW15 orders, 7/7 posts? (due 2026-10-04, 5 min)

### Week 4
- [ ] Call Ian. Refund the double charge, give him a new code. (due 2026-10-10, 30 min)
- [ ] Scorecard: $2,500+ so far? (due 2026-10-11, 5 min)
- [ ] Only if the booth is running: open coating deposits. Else skip. (due 2026-10-17, 30 min)
- [ ] Count the month. $5k+ → same routine, aim $10k in Nov. (due 2026-10-21, 30 min)

## Done

- 2026-09-22 — Plan written and committed. Store, timezone and rules set.

## Notes

- Scheduler rules: max one one-time `[Store]` task per weekday; weekends under 3 hours total. A
  slipped task moves to the next day — never stack two.
- Everything else in Notion is ignored until Nov 1.
- Coating only if the booth is running.
- Customers with open issues: Chow (order 1898), Karter, Ian (double charge).
